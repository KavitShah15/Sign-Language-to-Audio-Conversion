...
DATA_PATH = os.path.join("Folder-Name-to-store-data")

# define all required actions belows

gestures = np.array(['Thankyou']) # add the gestures
noSequences = 50
sequenceLength = 30
...
with mp_holistic.Holistic(min_detection_confidence=0.75, min_tracking_confidence=0.5) as holistic:
    for action in gestures:
        for sequence in range(noSequences):
